"""Ticket 06: no-dividend American call validation experiment."""
...
VALIDATION_CSV = PROJECT_ROOT / Path(
    "results/01_solver_validation/tables/ticket_06_no_dividend_american_call_validation.csv"
)
...
ERROR_FIGURE_PATH = PROJECT_ROOT / Path(
    "results/01_solver_validation/figures/"
...
def run_validation_cases(
    ...
    """Run the default no-dividend American call validation cases."""

    results: list[tuple[str, AmericanCNPSORResult]] = []
    for case in cases:
        _validate_no_dividend_case(case)
    ...
    validation_rows = [
        summarize_no_dividend_call_case(result) | {"case_name": case_name}
    ...
    for row in validation_rows:
        ...
        row["error_figure_created"] = "False"

    fine_result = results[-1][1]
    value_figure_created = create_value_comparison_figure(fine_result, VALUE_FIGURE_PATH)
    error_figure_created = create_theorem_error_figure(fine_result, ERROR_FIGURE_PATH)
    validation_rows[-1]["value_figure_created"] = str(value_figure_created)
    validation_rows[-1]["error_figure_created"] = str(error_figure_created)

    metadata = {
        "validation_csv": str(VALIDATION_CSV),
        "selected_spots_csv": str(SELECTED_SPOTS_CSV),
        "value_figure_path": str(VALUE_FIGURE_PATH),
        "error_figure_path": str(ERROR_FIGURE_PATH),
        "value_figure_created": str(value_figure_created),
        "error_figure_created": str(error_figure_created),
    }
    return validation_rows, selected_rows, metadata


def summarize_no_dividend_call_case(result: AmericanCNPSORResult) -> dict[str, str]:
    """Summarize theorem error, obstacle, and PSOR metadata for one call case."""

    _validate_result_is_no_dividend_call(result)
    ...
    return {
        ...
        "max_abs_american_european_error": _format_float(metrics.max_abs_error),
        "rmse_american_european_error": _format_float(metrics.rmse),
        "max_error_spot": _format_float(metrics.max_error_spot),
        "medium_to_fine_selected_spot_max_abs_diff": "",
        "value_figure_created": "False",
        "error_figure_created": "False",
    }
...
def create_value_comparison_figure(result: AmericanCNPSORResult, path: Path) -> bool:
    """Create the Ticket 06 American, European, and payoff comparison figure."""
    ...
    ax.set_title("Ticket 06 No-Dividend American Call Validation")
    ax.set_xlim(0.0, 2.0)
...
def create_theorem_error_figure(result: AmericanCNPSORResult, path: Path) -> bool:
    """Create the Ticket 06 American-minus-European theorem-error figure."""
    ...
    european_values = _european_call_values(result)
    x_values = result.spot_grid / result.K
    errors = result.values - european_values

    path.parent.mkdir(parents=True, exist_ok=True)
    fig, ax = plt.subplots(figsize=(7.2, 4.2))
    ax.plot(x_values, errors, label="American call - European call", linewidth=1.8)
    ax.axhline(0.0, color="black", linewidth=1.0, linestyle="--", label="Zero error")
    ax.set_xlabel("Moneyness S/K")
    ax.set_ylabel("Value difference")
    ax.set_title("Ticket 06 No-Dividend Call Theorem Error")
    ax.set_xlim(0.0, 2.0)
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
...
def main() -> tuple[list[dict[str, str]], list[dict[str, str]], dict[str, str]]:
    """Run Ticket 06 validation and write CSV/figure artifacts."""
    ...
    print(
        "error_figure_created="
        f"{metadata['error_figure_created']} path={ERROR_FIGURE_PATH}"
    )
    return validation_rows, selected_rows, metadata


def _validate_no_dividend_case(case: dict[str, Any]) -> None:
    if str(case.get("option_type", "")).lower() != "call":
        raise ValueError("Ticket 06 validation cases must use option_type='call'.")
    if float(case.get("q", float("nan"))) != 0.0:
        raise ValueError("Ticket 06 validation cases must use q = 0.0.")


def _validate_result_is_no_dividend_call(result: AmericanCNPSORResult) -> None:
    if result.option_type != "call":
        raise ValueError("Ticket 06 summaries require a call result.")
    if result.q != 0.0:
        raise ValueError("Ticket 06 summaries require q = 0.0.")
