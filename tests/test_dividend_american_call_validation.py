"""Ticket 07 tests for the dividend-paying American call validation experiment."""
...
SCRIPT_PATH = PROJECT_ROOT / "experiments" / "04_dividend_american_call_validation.py"
VALIDATION_CSV = (
    PROJECT_ROOT
    / "results"
    / "01_solver_validation"
    / "tables"
    / "ticket_07_dividend_american_call_validation.csv"
)
SELECTED_SPOTS_CSV = (
    PROJECT_ROOT
    / "results"
...
class DividendAmericanCallValidationExperimentTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        os.environ.setdefault("MPLCONFIGDIR", "/private/tmp/matplotlib-cache")
        os.environ.setdefault("FC_CACHEDIR", "/private/tmp/fontconfig-cache")
        cls.module = _load_ticket07_module()
    ...
    def test_american_call_values_dominate_european_call(self):
    ...
    def test_positive_american_minus_european_value_is_visible(self):
        max_american_minus_european = max(
            float(row["max_american_minus_european"]) for row in self.validation_rows
        )
        positive_counts = [
            int(row["positive_american_minus_european_node_count"])
        ...
        self.assertGreater(max_american_minus_european, 0.01)
        self.assertGreater(max(positive_counts), 0)
        self.assertGreater(max(selected_differences), 0.01)
    ...
    def test_medium_to_fine_comparison_is_recorded(self):
    ...
    def test_figures_created_when_matplotlib_available(self):
        if (
            ...
            or self.metadata["difference_figure_created"] != "True"
            or self.metadata["psor_figure_created"] != "True"
        ):
            self.skipTest("matplotlib unavailable in this runtime")

        self.assertTrue(VALUE_FIGURE_PATH.exists())
        self.assertTrue(DIFFERENCE_FIGURE_PATH.exists())
        self.assertTrue(PSOR_FIGURE_PATH.exists())
        self.assertGreater(VALUE_FIGURE_PATH.stat().st_size, 0)
        self.assertGreater(DIFFERENCE_FIGURE_PATH.stat().st_size, 0)
        self.assertGreater(PSOR_FIGURE_PATH.stat().st_size, 0)
        self.assertEqual(self.metadata["value_figure_path"], str(VALUE_FIGURE_PATH))
        self.assertEqual(self.metadata["difference_figure_path"], str(DIFFERENCE_FIGURE_PATH))
        self.assertEqual(self.metadata["psor_figure_path"], str(PSOR_FIGURE_PATH))
    ...
    def test_no_forbidden_boundary_greek_stress_dataset_or_neural_files_or_api(self):
        ...
        ticket07_paths = [
            path
            for path in PROJECT_ROOT.rglob("*ticket_07*")
    ...
    def test_new_python_files_include_ticket_07_docstrings(self):
        self.assertIn("Ticket 07", inspect.getdoc(self.module))

        test_doc = Path(__file__).read_text(encoding="utf-8")
        self.assertIn("Ticket 07", test_doc)


def _load_ticket07_module():
    spec = importlib.util.spec_from_file_location(
        "ticket07_dividend_american_call_validation", SCRIPT_PATH
    )
